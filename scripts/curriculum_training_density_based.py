#!/usr/bin/env python3
"""
Density-Based Curriculum Learning for Minesweeper RL

Uses mine density and different board sizes to create more gradual difficulty
progression, avoiding overfitting to specific board configurations.
"""
...
def density_based_curriculum_training():
    """Run density-based curriculum learning with gradual density increases."""
    
    print("🎓 Density-Based Curriculum Learning for Minesweeper RL")
    print("=" * 60)
    
    # Density-based curriculum stages
    # Format: (board_size, max_mines, timesteps, min_win_rate, description, target_density)
    curriculum_stages = [
        # Stage 1: Easiest possible (4x4, 1 mine) - 6.25% density
        ((4, 4), 1, 200000, 0.75, "Stage 1: 4x4 with 1 mine (6.25% density)", 0.0625),
        
        # Stage 2: Slightly higher density (5x5, 2 mines) - 8% density
        ((5, 5), 2, 300000, 0.65, "Stage 2: 5x5 with 2 mines (8% density)", 0.08),
        
        # Stage 3: Same density, different board (4x4, 2 mines) - 12.5% density
        ((4, 4), 2, 400000, 0.55, "Stage 3: 4x4 with 2 mines (12.5% density)", 0.125),
        
        # Stage 4: Higher density, larger board (6x6, 4 mines) - 11.1% density
        ((6, 6), 4, 500000, 0.50, "Stage 4: 6x6 with 4 mines (11.1% density)", 0.111),
        
        # Stage 5: Medium density (5x5, 4 mines) - 16% density
        ((5, 5), 4, 600000, 0.40, "Stage 5: 5x5 with 4 mines (16% density)", 0.16),
        
        # Stage 6: Higher density (6x6, 6 mines) - 16.7% density
        ((6, 6), 6, 700000, 0.35, "Stage 6: 6x6 with 6 mines (16.7% density)", 0.167),
        
        # Stage 7: Challenge density (5x5, 6 mines) - 24% density
        ((5, 5), 6, 800000, 0.25, "Stage 7: 5x5 with 6 mines (24% density)", 0.24),
        
        # Stage 8: Expert level (6x6, 8 mines) - 22.2% density
        ((6, 6), 8, 900000, 0.20, "Stage 8: 6x6 with 8 mines (22.2% density)", 0.222),
    ]
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = f"experiments/density_curriculum_results_{timestamp}.json"
    ...
    with open(results_file, 'w') as f:
        json.dump({
            "curriculum_summary": {
                ...
                "timestamp": timestamp,
                "approach": "density_based_progression"
            },
            "results": results
        }, f, indent=2)
    ...
    # Print final summary
    print(f"\n📈 Density-Based Curriculum Learning Summary")
    print("=" * 60)
...
if __name__ == "__main__":
    density_based_curriculum_training() 