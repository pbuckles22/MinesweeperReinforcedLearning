#!/usr/bin/env python3
"""
Enhanced Multi-Step DQN Training

Advanced training with refined hyperparameters and strategies:
- Extended training episodes (300-500 per stage)
- Optimized learning rates and exploration
...
class EnhancedMultiStepTrainer:
    """Enhanced trainer for multi-step DQN with advanced strategies."""
    
    def __init__(self):
        self.optimization_results = []
        
    def create_enhanced_agent(self, board_size: Tuple[int, int], config: Dict[str, Any]) -> MultiStepDQNAgent:
        """Create multi-step DQN agent with enhanced hyperparameters."""
    ...
    def train_with_enhanced_curriculum(self, episodes_per_stage: int = 300) -> Dict[str, Any]:
        """Train multi-step DQN with enhanced curriculum learning."""
        print("🚀 Enhanced Multi-Step DQN Training with Advanced Curriculum")
        print("=" * 75)
        print("Target: 85-90% win rate on 4x4 boards")
        print("Strategy: Extended training with optimized hyperparameters")
        print("=" * 75)
        
        # Enhanced curriculum stages with better progression
        curriculum_stages = [
            {
                'name': 'Stage 1: 2x2 Board (Foundation)',
                'board_size': (2, 2),
                'mine_count': 1,
                'episodes': episodes_per_stage,  # Full episodes for solid foundation
                'target_win_rate': 0.85,  # Higher target for foundation
                'config': {
                    'learning_rate': 0.001,  # Higher LR for faster learning on simple board
                    'discount_factor': 0.99,
                    'epsilon': 1.0,
                    'epsilon_decay': 0.998,  # Slower decay for better exploration
                    'epsilon_min': 0.1,  # Higher minimum for continued exploration
                    'replay_buffer_size': 100000,
                    'batch_size': 64,
                    'target_update_freq': 200,  # More frequent updates
                    'n_steps': 3,
                    'max_n_steps': 5
                }
            },
            {
                ...
                'board_size': (3, 3),
                'mine_count': 1,
                'episodes': episodes_per_stage * 2,  # Double episodes for complexity
                'target_win_rate': 0.75,
                'config': {
                    'learning_rate': 0.0005,  # Moderate LR for transfer learning
                    'discount_factor': 0.99,
                    'epsilon': 0.8,  # Start with lower epsilon
            ...
            },
            {
                ...
                'board_size': (4, 4),
                'mine_count': 1,
                'episodes': episodes_per_stage * 3,  # Triple episodes for complex board
                'target_win_rate': 0.85,
                'config': {
                    'learning_rate': 0.0002,  # Lower LR for fine-tuning
                    'discount_factor': 0.99,
                    'epsilon': 0.6,  # Start with even lower epsilon
                    'epsilon_decay': 0.9995,  # Extremely slow decay
                    'epsilon_min': 0.02,
        ...
        for i, stage in enumerate(curriculum_stages):
            ...
            print(f"   Learning Rate: {stage['config']['learning_rate']}")
            print(f"   Epsilon Decay: {stage['config']['epsilon_decay']}")
            print("-" * 70)
            ...
            # Create agent
            agent = self.create_enhanced_agent(stage['board_size'], stage['config'])
            
            # Train agent with more frequent evaluation
            ...
            # Extended evaluation for better accuracy
            eval_stats = evaluate_multistep_dqn_agent(agent, env, n_episodes=100)
            ...
            print(f"   Avg Reward: {eval_stats['mean_reward']:.2f}")
            print(f"   Avg Episode Length: {eval_stats['mean_length']:.1f}")
            
            # Save intermediate agent
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            agent_filename = f"enhanced_multistep_stage_{i+1}_{timestamp}.pth"
        ...
        # Print summary
        self._print_enhanced_summary(results, final_evaluation)
    ...
    def _comprehensive_evaluation(self, agent: MultiStepDQNAgent, final_stage: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"📊 Statistical Results (10 runs of 50 episodes each):")
    ...
    def _print_enhanced_summary(self, results: List[Dict[str, Any]], final_evaluation: Dict[str, Any]):
        """Print comprehensive enhanced optimization summary."""
        print("\n" + "=" * 75)
        print("🏆 ENHANCED MULTI-STEP DQN TRAINING SUMMARY")
        print("=" * 75)
        ...
        for i, result in enumerate(results):
            ...
            print(f"      N-steps: {stage['config']['n_steps']}")
            print(f"      LR: {stage['config']['learning_rate']}")
            print(f"      Epsilon Decay: {stage['config']['epsilon_decay']}")
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"enhanced_multistep_results_{timestamp}.json"
...
def main():
    """Run enhanced multi-step DQN training."""
    print("🧪 Enhanced Multi-Step DQN Training")
    print("=" * 55)
    
    try:
        # Create enhanced trainer
        trainer = EnhancedMultiStepTrainer()
        
        # Run enhanced training with curriculum
        results = trainer.train_with_enhanced_curriculum(episodes_per_stage=300)
        
        print(f"\n✅ Enhanced multi-step DQN training completed!")
