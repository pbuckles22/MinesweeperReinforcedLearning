#!/usr/bin/env python3
"""
Gradual Mine Count Curriculum Learning for Minesweeper RL

Keeps the same board size but varies mine count gradually to avoid observation
space mismatches while still providing progressive difficulty.
"""
...
def gradual_mines_curriculum_training():
    """Run curriculum learning with gradual mine count increases on same board size."""
    
    print("🎓 Gradual Mine Count Curriculum Learning for Minesweeper RL")
    print("=" * 60)
    
    # Gradual mine count curriculum stages (same board size)
    ...
    curriculum_stages = [
        ...
        # Stage 2: Slightly harder (4x4, 2 mines) - 12.5% density
        ((4, 4), 2, 300000, 0.60, "Stage 2: 4x4 with 2 mines (12.5% density)", 0.125),
        
        # Stage 3: Medium difficulty (4x4, 3 mines) - 18.75% density
        ((4, 4), 3, 400000, 0.45, "Stage 3: 4x4 with 3 mines (18.75% density)", 0.1875),
        
        # Stage 4: Harder (4x4, 4 mines) - 25% density
        ((4, 4), 4, 500000, 0.35, "Stage 4: 4x4 with 4 mines (25% density)", 0.25),
        
        # Stage 5: Very hard (4x4, 5 mines) - 31.25% density
        ((4, 4), 5, 600000, 0.25, "Stage 5: 4x4 with 5 mines (31.25% density)", 0.3125),
        
        # Stage 6: Expert (4x4, 6 mines) - 37.5% density
        ((4, 4), 6, 700000, 0.20, "Stage 6: 4x4 with 6 mines (37.5% density)", 0.375),
    ]
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = f"experiments/gradual_mines_curriculum_results_{timestamp}.json"
    ...
    with open(results_file, 'w') as f:
        json.dump({
            "curriculum_summary": {
                ...
                "timestamp": timestamp,
                "approach": "gradual_mines_progression"
            },
            "results": results
        }, f, indent=2)
    ...
    # Print final summary
    print(f"\n📈 Gradual Mine Count Curriculum Learning Summary")
    print("=" * 60)
...
if __name__ == "__main__":
    gradual_mines_curriculum_training() 