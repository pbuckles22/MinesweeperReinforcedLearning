#!/usr/bin/env python3
"""
Simple DQN 90%+ Win Rate Script

This script uses the ORIGINAL simple DQN agent (not enhanced) with the proven
hyperparameters we discovered, targeting 90%+ win rate.
"""
...
def create_simple_90_percent_agent(board_size: Tuple[int, int]) -> DQNAgent:
    """Create simple DQN agent with proven hyperparameters for 90%+ performance."""
    return DQNAgent(
...
def train_simple_90_percent(episodes: int = 2000, eval_freq: int = 100) -> Dict[str, Any]:
    """Train simple DQN agent to 90%+ win rate."""
    print(f"🎯 Simple DQN Training for 90%+ Win Rate")
    ...
    agent = create_simple_90_percent_agent((4, 4))
    
    print(f"✅ Using SIMPLE DQN with PROVEN hyperparameters:")
    ...
    print(f"   Architecture: Simple DQN (not enhanced)")
    print("-" * 60)
    
    start_time = time.time()
    
    # Use the proven training function
    training_stats = train_dqn_agent(env, agent, episodes, 1, eval_freq=eval_freq)
...
def comprehensive_evaluation(agent: DQNAgent, n_runs: int = 20, episodes_per_run: int = 100) -> Dict[str, Any]:
...
def main():
    """Run simple DQN training to achieve 90%+ win rate."""
    print("🏆 Simple DQN 90%+ Win Rate Push")
    print("=" * 55)
    
    try:
        # Run training with simple DQN and proven hyperparameters
        training_results = train_simple_90_percent(episodes=2000, eval_freq=100)
        ...
        print(f"\n🎯 Simple DQN Performance Assessment:")
        ...
        filename = f"simple_dqn_90_percent_results_{timestamp}.json"
        ...
        results = {
            ...
            'config': {
                'episodes': 2000,
                'eval_freq': 100,
                'target': '90%+ win rate (simple DQN with proven hyperparameters)'
            },
        ...
        print(f"\n❌ Error during simple DQN training: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = main()
    if success:
        print("\n🎉 Simple DQN training completed successfully!")
    else:
        print("\n💥 Simple DQN training failed. Please check the error messages above.")
        sys.exit(1) 