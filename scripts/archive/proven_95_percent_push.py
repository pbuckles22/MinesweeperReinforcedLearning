#!/usr/bin/env python3
"""
Proven 95%+ Win Rate Push Script

This script uses the PROVEN working configuration that achieved 72.3% win rate
and extends it with longer training to push for 95%+ performance.
"""
...
def create_proven_agent(board_size: Tuple[int, int]) -> EnhancedDQNAgent:
    """Create agent using the PROVEN working configuration (72.3% win rate)."""
...
def train_with_proven_config(episodes: int = 5000, eval_freq: int = 100) -> Dict[str, Any]:
    """Extended training using the proven configuration."""
    print(f"🚀 Starting Proven Configuration Training for 95%+ Win Rate")
    ...
    print(f"✅ Using PROVEN configuration (achieved 72.3% win rate):")
    ...
    print(f"\n✅ Proven configuration training completed!")
...
def main():
    """Run proven configuration training to push for 95%+ win rate."""
    print("🏆 Proven 95%+ Win Rate Push")
    print("=" * 55)
    
    try:
        # Run training with proven configuration
        training_results = train_with_proven_config(episodes=5000, eval_freq=100)
        ...
        filename = f"proven_95_percent_results_{timestamp}.json"
        ...
        results = {
            ...
            'config': {
                'episodes': 5000,
                'eval_freq': 100,
                'target': '95%+ win rate (proven configuration)'
            },
        ...
        print(f"\n❌ Error during proven 95%+ training: {e}")
        import traceback
        traceback.print_exc()
        return False
...
if __name__ == "__main__":
    success = main()
    if success:
        print("\n🎉 Proven 95%+ training completed successfully!")
    else:
        print("\n💥 Proven 95%+ training failed. Please check the error messages above.")
        sys.exit(1) 