...
Implements a more gradual difficulty progression with longer training on
intermediate stages to improve knowledge transfer and generalization.
"""
...
def refined_curriculum_training():
    """Run refined curriculum learning with gradual difficulty increases."""
    
    print("🎓 Refined Curriculum Learning for Minesweeper RL")
    print("=" * 60)
    
    # More gradual curriculum stages with longer training
    # Format: (board_size, max_mines, timesteps, min_win_rate, description)
    curriculum_stages = [
        # Stage 1: Easiest possible (4x4, 1 mine) - Master this first
        ((4, 4), 1, 200000, 0.75, "Stage 1: 4x4 with 1 mine (6.25% density)"),
        
        # Stage 2: Slightly harder (4x4, 2 mines) - Build on Stage 1
        ((4, 4), 2, 300000, 0.60, "Stage 2: 4x4 with 2 mines (12.5% density)"),
        
        # Stage 3: Same board, more mines (4x4, 3 mines)
        ((4, 4), 3, 400000, 0.45, "Stage 3: 4x4 with 3 mines (18.75% density)"),
        
        # Stage 4: Larger board, same density as Stage 2 (6x6, 4 mines)
        ((6, 6), 4, 500000, 0.40, "Stage 4: 6x6 with 4 mines (11.1% density)"),
        
        # Stage 5: Same board, more mines (6x6, 6 mines)
        ((6, 6), 6, 600000, 0.30, "Stage 5: 6x6 with 6 mines (16.7% density)"),
        
        # Stage 6: Challenge (6x6, 8 mines)
        ((6, 6), 8, 700000, 0.20, "Stage 6: 6x6 with 8 mines (22.2% density)"),
    ]
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = f"experiments/refined_curriculum_results_{timestamp}.json"
    ...
    # Print final summary
    print(f"\n📈 Refined Curriculum Learning Summary")
    print("=" * 60)
    print(f"{'Stage':<6} {'Board':<8} {'Mines':<6} {'Win Rate':<10} {'Density':<10} {'Status':<10}")
    print("-" * 60)
    ...
    for result in results:
        ...
        print(f"{stage:<6} {board:<8} {mines:<6} {win_rate:<10.1%} {density:<10.1%} {status:<10}")
